...
def pptx_to_images(pptx_path: str, output_dir: str, width=1920, height=1080) -> PPTConversionResult:
    ...
    Raises:
        ...
        Exception: 当转换过程中出现其他错误时
    """
    # 转换为绝对路径
    pptx_path = os.path.abspath(pptx_path)
    output_dir = os.path.abspath(output_dir)
    logger.info("开始将PPT转换为图片...")
    logger.info(f"PPT文件: {pptx_path}")
    ...
    try:
        ...
        logger.info(f"PPT原始尺寸: {slide_width} x {slide_height}, 宽高比: {aspect_ratio:.3f}")

        # 如果高度为0，根据宽度和原始比例计算
        if height == 0:
            height = int(width * aspect_ratio)
            logger.info(f"根据宽度自动计算高度: {height}px")

        logger.info(f"将导出为图片尺寸: {width}x{height}px")
        logger.info(f"幻灯片总数: {presentation.Slides.Count}")
        ...
        for i in range(1, presentation.Slides.Count + 1):
            ...
            logger.info(f"导出幻灯片 {i}/{presentation.Slides.Count} 到 {img_path}")
            ...
            try:
                ...
                logger.warning(f"使用指定尺寸导出失败 ({str(e)})，改用默认尺寸导出后再调整")
                ...
                try:
                    ...
                    logger.error(f"调整大小失败: {str(resize_error)}")
        ...
        try:
            ppt_app.Quit()
            logger.debug("PowerPoint应用程序已退出")
        except Exception as e:
            logger.error(f"退出PowerPoint时出错: {str(e)}")

        try:
            pythoncom.CoUninitialize()
            logger.debug("COM组件已释放")
        except Exception as e:
            logger.error(f"释放COM组件时出错: {str(e)}")
...
if __name__ == "__main__":
    ...
    logger.success(f"成功导出 {result.converted_slides}/{result.total_slides} 张图片到 {result.output_dir}")
    logger.info("按顺序排列的图片信息:")
    for info in result.image_info_list:
        logger.info(f"  幻灯片 {info.slide_number}: {info.filename} -> {info.filepath}")
